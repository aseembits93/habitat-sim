...
struct BatchRendererStandalone::State {
  ...
  State(const BatchRendererStandaloneConfiguration& configuration)
...
const void* BatchRendererStandalone::colorCudaBufferDevicePointer() {
  ...
  _state->framebuffer.read({{}, tileCount() * tileSize()}, _state->colorBuffer,
                           Mn::GL::BufferUsage::DynamicRead);

  /* Initialize the CUDA buffer from the GL buffer image if it's not already */
  if (!_state->cudaColorBuffer) {
    checkCudaErrors(cudaGraphicsGLRegisterBuffer(
        &_state->cudaColorBuffer, _state->colorBuffer.buffer().id(),
  ...
  CORRADE_INTERNAL_ASSERT(size == _state->colorBuffer.size().product() *
                                      _state->colorBuffer.pixelSize());
  return pointer;
}
...
const void* BatchRendererStandalone::depthCudaBufferDevicePointer() {
  ...
  /* Read to the buffer image, allocating it if it's not already. Can't really
     return a pointer directly to the renderbuffer because the returned device
     pointer is expected to be linearized. */
  _state->framebuffer.read({{}, tileCount() * tileSize()}, _state->depthBuffer,
                           Mn::GL::BufferUsage::DynamicRead);

  /* Initialize the CUDA buffer from the GL buffer image if it's not already */
  if (!_state->cudaDepthBuffer) {
    checkCudaErrors(cudaGraphicsGLRegisterBuffer(
        &_state->cudaDepthBuffer, _state->depthBuffer.buffer().id(),
        cudaGraphicsRegisterFlagsReadOnly));
  }
  ...
  std::size_t size;
  checkCudaErrors(cudaGraphicsResourceGetMappedPointer(
      &pointer, &size, _state->cudaDepthBuffer));
  CORRADE_INTERNAL_ASSERT(size == _state->depthBuffer.size().product() *
                                      _state->depthBuffer.pixelSize());
  return pointer;
}
#endif
