...
void BatchRenderer::addFile(const Cr::Containers::StringView filename,
  ...
  if (!(_state->flags & BatchRendererFlag::NoTextures)) {
  ...
  CORRADE_ASSERT(
      ...
      "BatchRenderer::addFile(): expected a file with exactly one mesh, got"
          << importer->meshCount(), );
  _state->mesh = Mn::MeshTools::compile(
  ...
  _state->textureTransformations = Cr::Containers::Array<TextureTransformation>{
  ...
  /* Fill initial projection data for each view. Will be uploaded afresh every
     draw. */
  _state->projections = Cr::Containers::Array<Mn::Shaders::ProjectionUniform3D>{
      Cr::DefaultInit, std::size_t(_state->tileCount.product())};
...

  {
    ...
    for (Mn::UnsignedLong root : scene->childrenFor(-1)) {
      ...
      _state->meshViewRangeForName.insert(
  ...
  _state->shader.bindMaterialBuffer(_state->materialUniform);
  if (!(_state->flags >= BatchRendererFlag::NoTextures))
    _state->shader.bindAmbientTexture(_state->texture);
}
...
std::size_t BatchRenderer::add(const Mn::UnsignedInt sceneId,
  ...
  const auto found = _state->meshViewRangeForName.find(
  ...
  for (std::size_t i = found->second.first(); i != found->second.second();
    ...
    arrayAppend(scene.textureTransformations, Cr::InPlaceInit)
        .setTextureMatrix(
            _state->textureTransformations[meshView.materialId].transformation)
...
void BatchRenderer::clear(const Mn::UnsignedInt sceneId) {
  CORRADE_ASSERT(sceneId < _state->scenes.size(),
                 "BatchRenderer::clear(): index"
                     << sceneId << "out of range for" << _state->scenes.size()
                     << "scenes", );
...
void BatchRenderer::draw(Mn::GL::AbstractFramebuffer& framebuffer) {
  ...
  for (std::size_t sceneId = 0; sceneId != _state->scenes.size(); ++sceneId)
    ...
    _state->scenes[sceneId].transformationUniform.setData(
        _state->scenes[sceneId].absoluteTransformations.exceptPrefix(1));

  for (Mn::Int y = 0; y != _state->tileCount.y(); ++y) {
    for (Mn::Int x = 0; x != _state->tileCount.x(); ++x) {
      framebuffer.setViewport(Mn::Range2Di::fromSize(
          Mn::Vector2i{x, y} * _state->tileSize, _state->tileSize));
      ...
      if (!(_state->flags & BatchRendererFlag::NoTextures))
        _state->shader.bindTextureTransformationBuffer(
            _state->scenes[scene].textureTransformationUniform);
      _state->shader.draw(_state->mesh,
                          stridedArrayView(_state->scenes[scene].drawCommands)
